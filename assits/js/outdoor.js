...
const cartDrop = document.getElementById("add-drop");
...
http.open('GET', '../data/outdoor.json', true);
http.send();
    ...
    if (this.readyState == 4 && this.status == 200) {
        ...
        for (let item of products) {
            ...
            <div class="card1">
                ...
                <div class="detailing-price-description">${item.detailprice}<span class="price" changing-original-currency="${item.p}">${item.p}</span>${item.Price}</div>
                <div class="button"><a href="#">View all details</a></div>
            </div>`;
        }
