...
let cardImage = document.querySelectorAll('.card');
let slideWidth = cardImage[0].clientWidth;
let a = 0;

function changeImage() {
    a++;
    if (a >= cardImage.length / 2) {
        index = 0;
        mainCard.style.transition = 'none';
        mainCard.style.transform = 'translateX(0px)';
        setTimeout(function() {
            mainCard.style.transition = 'transform 0.5s ease-in-out';
            changeImage();
        }, 200); 
    } else {
        mainCard.style.transform = 'translateX(-' + (slideWidth * a) + 'px)';
